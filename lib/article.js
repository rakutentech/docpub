...
module.exports = class Article extends Document {
     ...
     */
    constructor(path, config, section) {
        super(path, config, section);
        ...
        this._mdRenderer = new MarkdownRenderer(this, this._config);
        this._markdownHash = '';
        this._markdown = null;
        this._html = null;
    }
     ...
     */
    read() {
        logger.info(`Reading article ${this.path}`);
        ...
        return super.read()
            ...
            .then(resources => {
                this.setChildren(
                    resources.map(path => new Resource(path, this._config, this))
                );
    ...
    /**
     * Updates own and own children hash.
     * First collects hashes of all child resources.
     * Next updates own hash.
     * After this writes updated meta to disc.
     *
     * @returns {Promise} - promise to be fulfilled when meta of all resources is being updated
     */
    updateHash() {
        logger.trace(`Updating hash for article ${this.path}\n New hash: ${this.hash}`);

        const updates = this.resources.map(resource => resource.updateHash());

        return Promise.all(updates)
            .then(() => this._syncOwnHash())
            .then(() => this.meta.write());
    }
